...
public class FilterController {
    ...
    public ResponseEntity<List<String>> getAllCategories(@RequestHeader HttpHeaders token) {
        statusCode = validToken(token.getFirst(HttpHeaders.AUTHORIZATION));
        if (statusCode == 200) {
    ...
    @GetMapping("/brands")
    public ResponseEntity<List<String>> getAllBrands(@RequestHeader HttpHeaders token) {
        statusCode = validToken(token.getFirst(HttpHeaders.AUTHORIZATION));
        if (statusCode == 200) {
            return ResponseEntity
